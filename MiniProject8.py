...
time = 0
started = False
rock_group = set([])  # setup a group of rock
missile_group = set([])  # setup a group of missiles
explosion_group = set([])
...
class Sprite:
    ...
    def collide(self, other_object):
        ...
        # if distance of two centers < this sum, means collide
        return dist(self.pos, other_object.pos) < sum_of_radius
...
def group_collide(a_set, other_object):
    ...
    for each_item in set(a_set):
        # use a copy of the original to iterate
        if each_item.collide(other_object):
            # if collide
            new_explosion = Sprite(each_item.pos, [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(new_explosion)
            a_set.remove(each_item)
            return True
    return False
...
def group_group_collide(a_set, b_set):
    total_collide = 0
    for each_item in set(a_set):
        if group_collide(b_set, each_item):
            # collide
            a_set.discard(each_item)
            total_collide += 1
...
def rock_spawner():
    ...
    difficult_para = max(1, score / 10)

    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() * .6 * difficult_para - .3, random.random() * .6 * difficult_para - .3]
    rock_avel = random.random() * .2 - .1

    if started and len(rock_group) < 12:
        a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        # check the distance w/ ship
        if not a_rock.collide(my_ship):
            # ignore this one if it will be spawned on top of ship
